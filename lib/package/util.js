 ...
 */
var validateHilaryPackage = module.exports.validateHilaryPackage = function(dest, errCode) {
    errCode = errCode || 1;
...
var copyHilaryReleaseFiles = module.exports.copyHilaryReleaseFiles = function(dest, errCode) {
    ...
    CoreUtil.exec('cp app.js ' + destSrc);
    CoreUtil.exec('cp config.js ' + destSrc);
    CoreUtil.exec('cp LICENSE ' + destSrc);
    CoreUtil.exec('cp npm-shrinkwrap.json ' + destSrc);
    CoreUtil.exec('cp package.json ' + destSrc);
    CoreUtil.exec('cp README.md ' + destSrc);
    ...
    // distribution.
    CoreUtil.exec('cp -RLf node_modules ' + destSrc);

    // Remove all orig and rej files as they are useless and can trip up the debian packaging process
    CoreUtil.exec('find ' + destSrc + ' -name "*.orig" -exec rm {} \\;');
    CoreUtil.exec('find ' + destSrc + ' -name "*.rej" -exec rm {} \\;');
    ...
    CoreUtil.logSuccess('Successfully copied hilary release artifacts to '.text + destSrc.white);
};
 ...
 * @param  {Number}     [errCode]       The process error code to return on failure. Default: 1
 */
var saveBuildInfo = module.exports.saveBuildInfo = function(dest, version, systemInfo, errCode) {
    errCode = errCode || 1;

    var targetInfoPath = util.format('%s/build-info.json', dest);
...
var packageRelease = module.exports.packageRelease = function(srcDir, tarballDir, filename, errCode) {
    ...
    CoreUtil.exec(util.format('tar -czvf %s -C %s .', tarballPath, filename, srcDir), 'Error creating the distribution tar.gz file', errCode);
